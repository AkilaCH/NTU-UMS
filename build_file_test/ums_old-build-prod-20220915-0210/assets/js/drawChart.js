    ...
    function update(source) {
        ...
        function drawNodes(node, cx) {
            ...
            nodeEnter.append('rect')
                ...
                .attr('width', function (d) {
                    if (d.name.length >= characterLength) {
                        return Math.max(getWidthOfText(lineBreaker(d.name, characterLength)[0], "16px"), getWidthOfText(d.description, "14px")) + getWidthOfText(setUnit(d.meter), "24px") + 70;
                    } else {
                        return Math.max(getWidthOfText(d.name, "16px"), getWidthOfText(d.description, "14px")) + getWidthOfText(setUnit(d.meter), "24px") + 70;
                    }
                })
                .attr('height', function (d) { return d.name.length >= characterLength ? 44 : 30; })
                .style("fill", '#242C33')
            ...
            nodeEnter.append("text")
                ...
                .style("fill-opacity", 0)
                .append('tspan')
                .text(function (d) { return lineBreaker(d.name, characterLength)[0]; })
                .append('tspan')
                .text(function (d) { return lineBreaker(d.name, characterLength)[1] ? lineBreaker(d.name, characterLength)[1] : null; })
                .attr('dx', function (d) { return -getWidthOfText(lineBreaker(d.name, characterLength)[0], "10px"); })
                .attr('dy', '15px')
                .append('tspan')
                .text(function (d) { return lineBreaker(d.name, characterLength)[2] ? lineBreaker(d.name, characterLength)[2] : null; })
                .attr('dx', function (d) { return -getWidthOfText(lineBreaker(d.name, characterLength)[0], "10px"); })
                .attr('dy', '15px')
        ...
        function lineBreaker(str, maxLength) {
            ...
            for (let i = 0, length = res.length; i < length; i++) {
                lineTemp.push(res[i]);
                if(i % maxLength === 0 || (i % maxLength !== 0 && i == length - 1)){
                    temp.push(lineTemp.join(''));
                    lineTemp = [];
                }
            }
